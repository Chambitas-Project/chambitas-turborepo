...
export function StatCard({ title, value, trend, icon: _icon, color, onClick }: StatCardProps) {
  return (
    <Card
      onClick={onClick}
      className={cn(
        "bg-white border border-slate-100 shadow-sm rounded-xl overflow-hidden relative group",
        onClick && "cursor-pointer hover:shadow-md hover:border-slate-200 transition-all active:scale-[0.98]"
      )}
    >
      <div className={cn(
      ...
      <CardContent className="p-8 flex flex-col justify-between min-h-40 relative z-10">
