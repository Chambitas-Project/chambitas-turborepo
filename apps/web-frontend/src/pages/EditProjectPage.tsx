...
export function EditProjectPage() {
      ...
      try {
        ...
        if (p.skills) {
          setSelectedSkills(p.skills.map((s: any) => ({
            skill_id: s.skill_id,
            name: s.skill_name || s.name || s.skill_id,
            proficiency_level: s.min_proficiency || 3
          })));
        }
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await employerApi.updateProject(id!, {
      ...
      setError(err.response?.data?.message || "Ocurrió un error al actualizar el empleo. Revisa los datos.");
      setIsLoading(false);
    }
  };
  ...
  return (
    <DashboardLayout role="employer">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl lg:text-4xl font-black tracking-tight text-slate-900 mb-2">Editar Microtrabajo</h1>
          <p className="text-slate-500 font-medium text-lg">Modifica los detalles de tu proyecto.</p>
        </div>
