...
export class ApplicationsRepository {
  ...
  async findById(id: string): Promise<any | null> {
    const { data, error } = await this.client
      .from('applications')
      .select('*, projects(title, employer_id), student_profiles(full_name, academic_cycle, phone_number, avatar_url, careers(name), student_skills(proficiency_level, verified, skills(id, name)), users(email)), matches(score)')
      .eq('id', id)
      .is('deleted_at', null)
      .single();
  ...
  async findAll(filters: {
    ...
    let query = this.client
      .from('applications')
      .select('*, projects(title, employer_id), student_profiles(full_name, academic_cycle, phone_number, avatar_url, careers(name), student_skills(proficiency_level, verified, skills(id, name)), users(email)), matches(score)', { count: 'exact' })
      .is('deleted_at', null);
